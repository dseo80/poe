;Completed
;integrated all poe_hotkeys
;integrated pathofmacros
;removed dps calc from poems
;chicken is seperate

;STill TODO
;fix chicken to consider when right click menu opened for chatting
;tempest update overlay
;integrate poeitemlookup and poeiteminfo
...
Run PoE_ck.exe, , , PoePID2
Run PoE_Item\POE_ItemLookup.exe, , , PoePID3
Run PoE_Item\POE_ItemInfo.exe, , , PoePID4
...
/*
ArrayCount = 0
Loop, Read, HotkeyDesc.txt   ; This loop retrieves each line from the file, one at a time.
{
...
Loop, Read, HotkeyDesc.txt   ; This loop retrieves each line from the file, one at a time.
{
   ...
   Gui, 2:Add, Edit, Disabled vHotkeyDesc%A_Index% w260 xp+50 yp-5, %element%
...
ReadMe:
   MsgBox, ------------------------------INFO------------------------------`n`nUpdated by dseo
return
...
DoHotkey7:
	IfWinActive, Path of Exile ahk_class Direct3DWindowClass 
   	{
   	BlockInput On
	SendInput ^{Enter}Just a moment - in a map{Enter}
	BlockInput Off
	return
	}
return
...
DoHotkey8:
	IfWinActive, Path of Exile ahk_class Direct3DWindowClass 
   	{
   	BlockInput On
	SendInput ^{Enter}nty - looking for more/my buyout{Enter}
	BlockInput Off
	return
	}
return
...
DoHotkey11:
	...
	SendInput {Enter}
	Sendraw `/
	SendInput "afk Hello and welcome to my shop. I am playing from my phone so I may be slow, thanks for your patience. UNPRICED items I am looking for an offer. If I dont respond, I may be busy or your offer was too low. If I invite you please come to my hideout.Thanks for Looking."
	SendInput {Enter}
	BlockInput Off	
	return
	}
...
LoadSettings()
{
   ...
   Loop % NumHotkeys 
   {
      ...
      If A_index=1
      {
         IniRead, val, %ININame% , Hotkeys, hk_%A_Index%_hk,^h
      }
      If A_index=2
      {
         IniRead, val, %ININame% , Hotkeys, hk_%A_Index%_hk,^i
      }
      If A_index=3
      {
         IniRead, val, %ININame% , Hotkeys, hk_%A_Index%_hk,^t
      }
      If A_index=4
      {
      ...
      If A_index=7
      {
         IniRead, val, %ININame% , Hotkeys, hk_%A_Index%_hk,^1
      }
      If A_index=8
      {
         IniRead, val, %ININame% , Hotkeys, hk_%A_Index%_hk,^2
      }
      If A_index=9
      {
         IniRead, val, %ININame% , Hotkeys, hk_%A_Index%_hk,^3
      }
	        If A_index=10
      {
         IniRead, val, %ININame% , Hotkeys, hk_%A_Index%_hk,!1
      ...
      If A_index=11
      {
         IniRead, val, %ININame% , Hotkeys, hk_%A_Index%_hk,^a
      }
	  ...
	  If A_index=14
      {
         IniRead, val, %ININame% , Hotkeys, hk_%A_Index%_hk,F11
      }
	  If A_index=15
      {
         IniRead, val, %ININame% , Hotkeys, hk_%A_Index%_hk,^F11
	  ...
	  If A_index=16
      {
         IniRead, val, %ININame% , Hotkeys, hk_%A_Index%_hk,^F12
      }
